...
### View
- InputView
  - 사용자의 입력을 받는 기능을 담당한다.
  - [x] 자동차 이름 또는 반복 숫자 입력 받기.
  - [x] 입력만 받고 VO에서 가공 및 검증
...
- Car
  ...
  - [x] setPosition 메서드를 추가하지 않기
  - [x] 자동차의 이름을 출력할 수 있어야 함
  - [x] 생성자에서 이름은 5자 이하만 가능에 대한 검증 로직
  - [x] 자동차 자체의 행위 구현
- Cars
  - Car VO를 포함한 1급 컬렉션, car들에 대한 행위를 갖고 있음
  - [x] Collection 외, 다른 멤버 변수가 없어야 한다
  - [x] 입력을 받은 후, Car 객체들의 불변성을 유지하기(추가나 삭제 금지)
  - [x] 상태와 행위를 한 곳에서 관리하기
  - [x] 경기가 종료되면, 우승자 계산
- IterationCount
  - 사용자가 몇 번 이동할 것인지 입력에 대한 VO
  - [x] 숫자 검증 - 시도 횟수는 숫자여야 한다.
  - [x] 1 이상 값인지 검증 - 시도 횟수는 양수여야 한다.
